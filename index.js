...
app.get("/", function (request, response) {
    response.send("これはScrapBoxをインデックス登録するためのサイトです。");
});

app.get("/update", function (request, response) {
    genSitemap();
    response.send("Sitemap Updated!");
    console.log("gasによりアップデートされました。");
});

app.get("/sitemap.xml", (req, res) => {
    res.set("Content-Type", "text/xml");
    res.send(
        fs.readFileSync(require("path").resolve(__dirname, "./sitemap.xml"), "utf8")
    );
});
...
app.get("/sitemap_index.xml", (req, res) => {
    ...
    res.send(
        fs.readFileSync(
            require("path").resolve(__dirname, "./sitemap_index.xml"),
            "utf8"
        )
    );
});
...
app.get("/scrapbox2/:title", (req, res) => {
    ...
    res.redirect(
        301,
        `https://www.scrapbox.io/yamato3010/${encodeURIComponent(title)}`
    );
});
...
function genSitemap() {
    ...
    client.fetch(RSS, {}).then(function (result) {
        if (result.error) {
            console.log("error");
            return;
        }
        result.$("item").each(function (idx) {
            ...
            var url =
                "<url><loc>https://scrapbox-index.glitch.me/scrapbox/" +
                keyArr[4] +
                "</loc><lastmod>" +
                y +
                "-" +
        ...
        map =
            map +
            "<url><loc>https://scrapbox-index.glitch.me/</loc><lastmod>2022-10-17</lastmod></url></urlset>";
        console.log(map);
